...
import { footerLinks, whyUs } from '../data/mock';
...
const Footer = () => {
  ...
  const submit = (e) => {
    ...
    toast.success('Welcome to the Verdant Clothier family! Check your inbox for 20% off.');
    setEmail('');
  };
  ...
  return (
    <footer className="mt-16">
      {/* Why Us */}
      <section className="bg-[#f0f5f1] py-14">
        <div className="max-w-[1480px] mx-auto px-4 lg:px-10">
          <h3 className="text-center font-display text-[28px] md:text-[34px] mb-10">Why Verdant Clothier?</h3>
              ...
              return (
                <div key={w.title} className="flex flex-col items-center text-center">
                  <div className="w-16 h-16 rounded-full bg-white border border-neutral-200 flex items-center justify-center mb-4">
                    <Icon size={26} className="text-[#2d5a3d]" />
                  </div>
      ...
      <section className="py-14 border-t border-neutral-200">
        <div className="max-w-[1480px] mx-auto px-4 lg:px-10 grid grid-cols-2 md:grid-cols-5 gap-10">
          <div className="col-span-2">
            <div className="font-display text-[26px]">verdant clothier</div>
            <p className="text-[13px] text-neutral-600 mt-4 leading-relaxed max-w-xs">A women-owned brand celebrating vintage-inspired fashion from the 1920s through Y2K. Sizes XS–5X.</p>
            <div className="flex items-center gap-4 mt-5 text-neutral-700">
              <a aria-label="instagram" href="#" className="hover:text-[#2d5a3d]"><Instagram size={20} /></a>
              <a aria-label="facebook" href="#" className="hover:text-[#2d5a3d]"><Facebook size={20} /></a>
              <a aria-label="youtube" href="#" className="hover:text-[#2d5a3d]"><Youtube size={20} /></a>
            </div>
          </div>
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-2.5">
                {links.map((l) => (
                  <li key={l}><Link to={`/collections/${l.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="text-[13px] text-neutral-600 hover:text-[#2d5a3d] link-underline">{l}</Link></li>
                ))}
              </ul>
            </div>
          ))}
      ...
      <section className="border-t border-neutral-200 py-6">
        <div className="max-w-[1480px] mx-auto px-4 lg:px-10 flex flex-col md:flex-row items-center justify-between gap-3 text-[12px] text-neutral-500">
          <div>© {new Date().getFullYear()} Verdant Clothier. All rights reserved.</div>
